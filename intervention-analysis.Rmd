...
6. MCAP confidence intervals for estimates 
...
max_days <- as.integer(prop$max_days)
...
data_all=subset(data,time <= max_days)
data_all$.id<-"data"
...
rem_low_count_simulations <- function(sdat, n) {
  ...
  for (i in 1:n) {
    ...
    if (any(wcases[1:(total_weeks-wlim)] == 0)) {
...
mdl_int  %>%
  simulate(params=theta_opt, nsim=9, format="data.frame", include.data=FALSE, times=1:max_days) -> sim_data
sim_data <- bind_rows(data_all, sim_data)
...
ggsave(file.path(plotting_folder, "3-simulations_int200.pdf"))
```
...
mdl_int  %>%
  simulate(params=theta_noq, nsim=9, format="data.frame", include.data=FALSE, times=1:max_days) -> sim_data_noq
sim_data_noq <- bind_rows(data_all, sim_data_noq)
...
ggsave(file.path(plotting_folder, "3-simulations_noint200.pdf"))
```
...
mdl_int %>% 
  simulate(params=theta_opt, nsim=num_sims, format = "data.frame", include.data=FALSE, times=1:max_days) -> sim_data_int
sim_data_int <- bind_rows(data_all, sim_data_int)
sim_data_int <- rem_low_count_simulations(sim_data_int, num_sims)

mdl_int %>% 
  simulate(params=theta_noq, nsim=num_sims, format = "data.frame", include.data=FALSE, times=1:max_days) -> sim_data_noint
sim_data_noint <- bind_rows(data_all, sim_data_noint)
sim_data_noint <- rem_low_count_simulations(sim_data_noint, num_sims)

  
# Adding cumulative counts
cobs <- cumulative_curve(data_all, max_days)

all_csum <- calc_cumulative_counts(sim_data_int, max_days)
all_csum <- c(cobs, all_csum)
sim_data_int <- cbind(sim_data_int, cumulative=all_csum)

all_csum <- calc_cumulative_counts(sim_data_noint, max_days)
all_csum <- c(cobs, all_csum)
...
case_area_plot(sim_data_int, "4-simulated_percentiles-with_int200.pdf")
case_area_plot(sim_data_noint, "4-simulated_percentiles-wout_int200.pdf")
```
...
csim_int <- cumulative_curve(sim_int, max_days)
...
csim_noint <- cumulative_curve(sim_noint, max_days)

df <- data.frame('time' = seq(1, max_days), 
                 'obs_data' = cobs,
...
ggsave(file.path(plotting_folder, "4-cumulative_cases_median-comparison200.pdf"))
```
...
cumulative_area_plot(sim_data_int, cmax, "4-cumulative_cases_percentiles-comparison-with_int200.pdf")

cmax <- 0.8 * max(sim_data_noint$cumulative)
cumulative_area_plot(sim_data_noint, cmax, "4-cumulative_cases_percentiles-comparison-wout_int200.pdf")
```
