...
mdl_int %>%
  simulate(params=theta_opt, nsim=9, format="data.frame", include.data=FALSE, times=1:200) %>% bind_rows(data_all) -> sim_data
...
ggsave(file.path(plotting_folder, "3-simulations_long.pdf"))
```
...
cobs <- cumulative_curve(data_all, 110)

all_csum <- calc_cumulative_counts(sim_data_int, 110)
...
mdl_int %>% 
  simulate(params=theta_opt, nsim=num_sims, format = "data.frame", include.data=FALSE, times=1:200) -> sim_data_int
...
# Adding cumulative counts
#cobs <- cumulative_curve(data_all, 200)

#all_csum <- calc_cumulative_counts(sim_data_int, 200)
#all_csum <- c(cobs, all_csum)
#sim_data_int <- cbind(sim_data_int, cumulative=all_csum)
```
...
ggsave(file.path(plotting_folder, "4-cumulative_cases_long.pdf"))
```
